...
export class Simulation {
  ...
  constructor(seed: number) {
    ...
    this.fortSeq = this.world.forts.length;
  ...
  private replan(c: Character): void {
    ...
    // NPC: 欲求に基づくゴール選択
    if (c.stats.hp < c.stats.maxHp * 0.4 || c.needs.hunger > 12) {
      const city = this.nearestCity(c.pos);
      c.goalPos = { x: city.pos.x, y: city.pos.y };
      c.state = 'seekCity';
      c.inventory.food = Math.max(c.inventory.food, 5);
      c.needs.hunger = 0;
      return;
    }
    // 帰属度で行動
    if (c.cityAttachment > 0.5) {
      const home = this.world.cities[c.homeCityId] ?? this.nearestCity(c.pos);
      c.goalPos = { x: home.pos.x + this.rng.range(-200, 200), y: home.pos.y + this.rng.range(-200, 200) };
      c.state = 'seekCity';
    } else {
      const target = this.rng.chance(0.5) ? this.rng.pick(this.world.cities).pos : this.rng.pick(this.world.villages).pos;
      c.goalPos = { x: target.x + this.rng.range(-150, 150), y: target.y + this.rng.range(-150, 150) };
      c.state = 'wander';
    }
  }
